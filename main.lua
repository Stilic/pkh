...
function self.build(repository, name, skip_dependencies)
    ...
    os.execute(
        "bwrap --unshare-all --clearenv --setenv PATH /usr/libexec/gcc/x86_64-pc-linux-musl/14.2.0:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin --chdir /pkh --dev /dev --tmpfs /tmp --ro-bind /bin /bin --ro-bind /lib /lib --ro-bind /sbin /sbin --ro-bind /usr /usr --ro-bind . /pkh --bind " ..
        build_path ..
        " /pkh/" ..
