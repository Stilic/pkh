...
function self.build_cmake(options, source, project, cflags, cppflags, ldflags, cc, cxx)
    if options then
        options = " " .. options
    else
        options = ""
    ...
    return function()
        ...
        os.execute("cmake --build " .. build_dir .. " --target install" .. system.get_make_jobs())
    end
end
...
function self.build_kconfig(source, cflags, cppflags)
    ...
    return function()
        ...
        local flags = self.get_flags(cflags, cppflags)
        os.execute(flags .. " make olddefconfig")
        os.execute("KCONFIG_NOTIMESTAMP=1 " .. flags .. " make" .. system.get_make_jobs())
    end
end
